// Configuration for Extension Web Store links
// Update these URLs to point to your store pages once they are approved.
// For now, they point to the custom local /download page.

export const CHROME_EXTENSION_URL = "/download";
export const FIREFOX_EXTENSION_URL = "/download";

// Store URLs for reference
export const CHROME_STORE_URL = "https://chromewebstore.google.com/detail/lazeedev-ai-job-autofill-cold-dm/hkompooiicoamiambpjhbbmimjefgiii";
export const FIREFOX_STORE_URL = "https://addons.mozilla.org/en-US/firefox/addon/lazeedev/";

// Direct Cloudflare download links for manual install ZIP/XPI files
// Replace these with your actual Cloudflare R2 / Worker links
export const CHROME_ZIP_URL = "https://pub-your-id.r2.dev/lazee-chrome.zip";
export const FIREFOX_ZIP_URL = "https://pub-your-id.r2.dev/lazee-firefox.xpi";

